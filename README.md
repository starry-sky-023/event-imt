...
同样的提供了 `umd` 和 `iife` 版本, 只需将 `cjs` 换成对应的 `umd` 或 `iife`, 导入后将直接挂载到全局对象上, 你可以通过全局对象中的 `EventImt.Bus` 使用

```js
import 'event-imt/dist/index.umd.js'
const bus = new EventImt.Bus()
```
...
- options 配置对象 [可选]
    ...
    - eventMap Map 实例, 直接传入一个 Map 实例, 该实例将被用作事件存储, 而非内部创建 [可选]
    - ctx 实例上下文 hook, 接收一个函数, 将在初始化前执行 [可选]
	- onError 当执行事件回调函数发生错误时的钩子函数 [可选]
	- onWarning 当事件不存在发出警告时的钩子函数 [可选]
...
```ts
import type { InterfaceToType } from 'event-imt'
interface BusEvent {
	sum?(a: number, b: number): number
	set?(str: 'a' | 'b'): string
	send?(msg: any): void
...
const bus = new Bus<InterfaceToType<BusEvent>>()
bus.on('sum', (a, b) => {}) // 类型推导
bus.emit('sum', 1, 2) // 类型推导
```
...
class MyBus extends Bus<InterfaceToType<BusEvent>> {}
